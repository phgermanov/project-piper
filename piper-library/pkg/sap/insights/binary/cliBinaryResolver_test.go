...
func TestCLIBinaryResolver_InstallDevOpsInsightsCli(t *testing.T) {
	t.Parallel()
	type testCase struct {
		name            string
		authToken       string
		version         string
		prepareResolver func(self *testCase, t *testing.T) CLIBinary
		wantErr         bool
	}
	pathToBin := "/tmp/devops-insights"
	tests := []*testCase{
		{
			name:      "canonical install",
			authToken: "90234jasd",
			version:   "", // empty version invokes latest release
			prepareResolver: func(self *testCase, t *testing.T) CLIBinary {
				// CLI release Resolver will be called with the supplied auth token
				releaseResolver := NewMockCliReleaseResolver(t)
				releaseResolver.On("GetLatestReleaseURL").Return("myURL", nil)
				releaseResolver.On("CreateHTTPHeader").Return(http.Header{
					"Authorization": []string{"token 90234jasd"},
				...
				fileDownloader.On("DownloadFile", "myURL", mock.Anything, mock.Anything, mock.Anything).Return(
					func(url string, filename string, header http.Header, cookies []*http.Cookie) error {
						...
						if !strings.HasSuffix(filename, pathToBin) {
							t.Errorf("Expected download location to end with %s but it does not: %s", pathToBin, fileDownloader)
						}
				...
				permEditor.On("Chmod", mock.Anything, os.FileMode(0755)).Return(
					func(path string, mode os.FileMode) error {
						if !strings.HasSuffix(path, pathToBin) {
							t.Errorf("expected -rwxr-xr-x file permissions to be set on the binary %s but it was set on %s", pathToBin, path)
						}
						return nil
					},
				)
		...
		},
		{
			...
			prepareResolver: func(self *testCase, t *testing.T) CLIBinary {
				// CLI release Resolver will be called with the supplied auth token
				releaseResolver := NewMockCliReleaseResolver(t)
				releaseResolver.On("GetReleaseVersionURL", "version").Return("myURL", nil)
				releaseResolver.On("CreateHTTPHeader").Return(http.Header{
					"Authorization": []string{"token 90234jasd"},
					"Accept":        []string{"application/octet-stream"},
				})
	...
	for _, c := range tests {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			err := resolver.InstallDevOpsInsightsCli(pathToBin, testCase.version)

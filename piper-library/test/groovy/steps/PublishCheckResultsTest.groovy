...
class PublishCheckResultsTest extends BasePiperTest {
    ...
    void testPublishWithDefaultSettings() throws Exception {
        jsr.step.publishCheckResults(script: nullScript)
    ...
    void testPublishForJavaWithDefaultSettings() throws Exception {
        jsr.step.publishCheckResults(script: nullScript, pmd: true, cpd: true, findbugs: true, checkstyle: true)
    ...
    void testPublishForJavaScriptWithDefaultSettings() throws Exception {
        jsr.step.publishCheckResults(script: nullScript, eslint: true)
        ...
        // ensure default patterns are set
        assertEquals("ESLint default pattern not set", '**/target/eslint.checkstyle.xml', publisherStepOptions['WarningsPublisher']['parserConfigurations'][0]['pattern'])
        // ensure nothing else is published
        assertTrue("PmdPublisher options not empty", publisherStepOptions['PmdPublisher'] == null)
        assertTrue("DryPublisher options not empty", publisherStepOptions['DryPublisher'] == null)
        assertTrue("FindBugsPublisher options not empty", publisherStepOptions['FindBugsPublisher'] == null)
    ...
    @Test
    void testPublishForFioriWithDefaultSettings() throws Exception {
        jsr.step.publishCheckResults(script: nullScript, fiori: true)

        assertTrue("AnalysisPublisher options not set", publisherStepOptions['AnalysisPublisher'] != null)
        assertTrue("WarningsPublisher options not set", publisherStepOptions['WarningsPublisher'] != null)
        assertTrue("WarningsPublisher parser configuration number not correct", publisherStepOptions['WarningsPublisher']['parserConfigurations'].size() == 1)
        // ensure correct parser is set set
        assertEquals("Fiori parser not correct", 'Fiori Analysis Parser', publisherStepOptions['WarningsPublisher']['parserConfigurations'][0]['parserName'])
        // ensure default patterns are set
        assertEquals("Fiori default pattern not set", '**/target/fiori-quality-report.xml', publisherStepOptions['WarningsPublisher']['parserConfigurations'][0]['pattern'])
        // ensure nothing else is published
        assertTrue("PmdPublisher options not empty", publisherStepOptions['PmdPublisher'] == null)
        assertTrue("DryPublisher options not empty", publisherStepOptions['DryPublisher'] == null)
        assertTrue("FindBugsPublisher options not empty", publisherStepOptions['FindBugsPublisher'] == null)
    ...
    void testPublishForPythonWithDefaultSettings() throws Exception {
        jsr.step.publishCheckResults(script: nullScript, pylint: true)
    ...
    void testPublishNothing() throws Exception {
        jsr.step.publishCheckResults(script: nullScript, aggregation: false)
    ...
    void testPublishNothingExplicitFalse() throws Exception {
        jsr.step.publishCheckResults(script: nullScript, pmd: false)
    ...
    void testPublishNothingImplicitTrue() throws Exception {
        jsr.step.publishCheckResults(script: nullScript, pmd: [:])
    ...
    void testPublishNothingExplicitActiveFalse() throws Exception {
        jsr.step.publishCheckResults(script: nullScript, pmd: [active: false])
    ...
    void testPublishWithCustomPattern() throws Exception {
        jsr.step.publishCheckResults(script: nullScript, eslint: [pattern: 'my-fancy-file.ext'], pmd: [pattern: 'this-is-not-a-patter.xml'])
    ...
    void testPublishWithArchive() throws Exception {
        jsr.step.publishCheckResults(script: nullScript, archive: true, eslint: true, pmd: true, cpd: true, findbugs: true, checkstyle: true, fiori: true)

        assertTrue("ArchivePatterns number not correct", archiveStepPatterns.size() == 6)
        ...
        assertTrue("ArchivePatterns contains no ESLint pattern", archiveStepPatterns.contains('**/target/eslint.checkstyle.xml'))
        assertTrue("ArchivePatterns contains no Fiori pattern", archiveStepPatterns.contains('**/target/fiori-quality-report.xml'))
    }

    @Test
    void testPublishWithPartialArchive() throws Exception {
        jsr.step.publishCheckResults(script: nullScript, archive: true, eslint: [archive: false], pmd: true, cpd: true, findbugs: true, checkstyle: true)
        ...
        assertTrue("ArchivePatterns contains ESLint pattern", !archiveStepPatterns.contains('**/target/eslint.checkstyle.xml'))
    }

    @Test
    void testPublishWithDefaultThresholds() throws Exception {
        jsr.step.publishCheckResults(script: nullScript, pmd: true)
    ...
    void testPublishWithThresholds() throws Exception {
        jsr.step.publishCheckResults(script: nullScript, aggregation: [thresholds: [fail: [high: '10']]], pmd: true)

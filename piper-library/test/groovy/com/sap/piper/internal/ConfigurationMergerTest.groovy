...
class ConfigurationMergerTest {
    ...
    void testReadConfigInsideMerge() {
        ...
        def config = [globalPipelineEnvironment: [configuration: [steps:[myStep:[overwrite: 'y', key1:'value1']]]]]
    ...
    @Test
    void testMergeMapParameterNoDefault() {
        Map defaults = [:]
        Map parameters = [appContainer: [githubOrg: 'ghOrg', githubRepo: 'ghRepo']]
        Set parameterKeys = ['appContainer']
        Set configurationKeys = []
        Map merged = ConfigurationMerger.merge(parameters, parameterKeys, [:], configurationKeys, defaults)
        Assert.assertEquals('ghOrg', merged.appContainer.githubOrg)
        Assert.assertEquals('ghRepo', merged.appContainer.githubRepo)
    }
}

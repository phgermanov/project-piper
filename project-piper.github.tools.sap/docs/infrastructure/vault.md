...
!!! tip ""
    ...
    === "Azure DevOps"
        ...
        * Take the **AppRole ID** and create an Azure Pipeline variable `hyperspace.vault.roleId`
        * Take the **AppRole Secret ID** and create an Azure Pipeline variable `hyperspace.vault.secretId`

        ![Create two azure pipeline variables](../images/azure-vault-variables.png)

    === "GitHub Actions"

        * Take the **AppRole ID** and [create a repository secret](https://docs.github.com/en/codespaces/managing-codespaces-for-your-organization/managing-secrets-for-your-repository-and-organization-for-github-codespaces#adding-secrets-for-a-repository) `PIPER_VAULTAPPROLEID`
        * Take the **AppRole Secret ID** and [create a repository secret](https://docs.github.com/en/codespaces/managing-codespaces-for-your-organization/managing-secrets-for-your-repository-and-organization-for-github-codespaces#adding-secrets-for-a-repository)  `PIPER_VAULTAPPROLESECRETID`

        ![Create two github repository variables](../images/actions-vault-variables.png)
...
!!! tip ""
    ...
    === "Jenkins"
        ...
        [Create a Vault Token](https://www.vaultproject.io/api/auth/token#create-token).

        In order to use a Vault Token for authentication you need to store the Vault token inside your Jenkins instance as shown below.
...
general:
  ...
  vaultPath: 'kv/my-pipeline' # the path under which your secrets are stored
  ...
```
...
!!! tip "Additional orchestrator specific configuration"

    === "Jenkins"

        When running a step via the Jenkins library you can use use `vaultAppRoleTokenCredentialsId` and `vaultAppRoleSecretTokenCredentialsId` or `vaultTokenCredentialsId` in your `config.yml` to point to the credentials you created in the section [AppRole Authentication](#approle-authentication) or [Token Authentication](#token-authentication).

        ```yaml
        # .pipeline/config.yml

        general:
          vaultAppRoleTokenCredentialsId: <your credentials Id for the AppRole Id>
          vaultAppRoleSecretTokenCredentialsId: <your credentials Id for the AppRole secret Id>
        ```

    === "Azure DevOps"

        _no additional configuration required_
...
The first path where test credentials are found will be used.
...
!!! note "Using a custom prefix for test credentials"
...
See [here](https://pages.github.tools.sap/SAP-vault) for more information about Vault @ SAP.
